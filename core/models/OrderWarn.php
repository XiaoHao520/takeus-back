...
class OrderWarn extends Model
{
    ...
    public function notify()
    {
        ...
        try {
            ...
            if ($is_sms == 1) {
                ...
                $error=new Error();
                $error->store_id=$order->store_id;
                $error->details='正在调用短信';
                $error->save();

                  Sms::sendNewOrder($order->store_id,$order->id);
    ...
    {
        /* @var Order $order */
        $order = $this->order = RequirementOrder::findOne(['id' => $this->order_id]);
