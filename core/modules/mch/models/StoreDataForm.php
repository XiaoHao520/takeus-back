...
class StoreDataForm extends Model
{
    ...
    public function search()
    {
        ...
        $data = [
            'all_count' => [
                'all' => RequirementOrder::find()->where($condition)->count('1'),
                '1day' => RequirementOrder::find()->where($condition)->andWhere([
                    'AND',
                    ['>=', 'addtime', strtotime(date('Y-m-d 00:00:00'))],
                    ['<=', 'addtime', strtotime(date('Y-m-d 23:59:59'))],
                ])->count('1'),
                '7day' => RequirementOrder::find()->where($condition)->andWhere([
                    'AND',
                    ['>=', 'addtime', strtotime(date('Y-m-d 23:59:59')) - 86400 * 7],
                    ['<=', 'addtime', strtotime(date('Y-m-d 23:59:59'))],
                ])->count('1'),
                '30day' => RequirementOrder::find()->where($condition)->andWhere([
                    'AND',
            ...
            'money' => [
                'day' => RequirementOrder::find()->where($condition_m)->andWhere([
                    'AND',
                    ['>=', 'addtime', strtotime(date('Y-m-d 00:00:00'))],
                    ['<=', 'addtime', strtotime(date('Y-m-d 23:59:59'))],
                    ['is_pay' => 1]
                ])->select(['sum(pay_price)'])->scalar(),
                'month' => RequirementOrder::find()->where($condition_m)->andWhere([
                    'AND',
                    ['month(FROM_UNIXTIME(addtime))' => date('m')],
                    ['is_pay' => 1]
                ])->select(['sum(pay_price)'])->scalar(),
                'all' => RequirementOrder::find()->where($condition_m)->andWhere([
                    'is_pay' => 1
                ])->select(['sum(pay_price)'])->scalar(),
                'refuse_1' => RequirementOrder::find()->where($condition_m)->andWhere([
    ...
    public function newSearch()
    {
        $data = [
            ...
            'panel_2' => [
                ...
                'order_service_count' => $this->getOrderRefundingCount(),
            ],
            'panel_3' => [
                'data_1' => [
    ...
    {
        $goods_count = Photographer::find()->where([
            'store_id' => $this->store_id,
            'is_delete' => 0,
        ])->count();
        $order_count = RequirementOrder::find()->where([
            'store_id' => $this->store_id,
            'is_delete' => 0,
            'is_cancel'=>0,
            'is_pay'=>1
    ...
    {
        $count = RequirementOrder::find()->where([
            'store_id' => $this->store_id,
            'is_delete' => 0,
            'is_pay' => 1,
            'status' => 1,
    ...
    {
        $query = Photographer::find()->alias('od')
            ->leftJoin(['o' => RequirementOrder::tableName()], 'od.id=o.photographer_id')
            ->where([
                'o.is_pay' => 1,
                'o.is_delete' => 0,
                'od.is_delete' => 0,
        ...
        $count = $query->select('SUM(od.id) AS num')->asArray()->one();
    ...
    {
        $query = RequirementOrder::find()->alias('o')
            ->where([
                'o.is_pay' => 1,
                'o.is_delete' => 0,
                'o.store_id' => $this->store_id,
    ...
    public function getGoodsSaleTopList($start_time = null, $end_time = null, $limit = 10)
    {
        ...
        return $query->select('od.name,SUM(od.id) AS num')->groupBy('od.id')->orderBy('num DESC')->limit($limit)->asArray()->all();
    }
    ...
    {
        $list = RequirementOrder::find()->alias('o')->leftJoin(['u' => User::tableName()], 'o.user_id=u.id')
            ->where([
                'o.store_id' => $this->store_id,
                'o.is_pay' => 1,
                'o.is_delete' => 0,
