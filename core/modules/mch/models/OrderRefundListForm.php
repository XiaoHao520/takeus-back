...
class OrderRefundListForm extends Model
{
    ...
    public function search()
    {
        ...
        $query = OrderRefund::find()->alias('or')
            ...
            ->where(['or.store_id' => $this->store_id, 'or.is_delete' => 0, 'o.mch_id' => 0,]);
        if ($this->status == 0) {
...
//            ]);
        ...
        $list = $query->select('or.id order_refund_id,o.id order_id,o.order_no,o.name,o.mobile,o.address,o.pay_type,u.nickname,u.id user_id,g.name goods_name,g.id goods_id,or.addtime,od.num,od.attr,od.total_price,or.type refund_type,or.status refund_status,or.desc refund_desc,or.pic_list refund_pic_list,or.refund_price,or.refuse_desc refund_refuse_desc')->asArray()->all();
