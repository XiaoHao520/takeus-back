...
use app\modules\mch\models\PhotographerLevelForm;
use app\modules\mch\models\PostageRulesEditForm;
use app\modules\mch\models\RequirementForm;
...
use app\modules\mch\models\TaocanForm;
...
class StoreController extends Controller
{
    ...
    {
        $cat_list = Requirement::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->orderBy('addtime DESC')->all();
     ...
     * 分类编辑
     */
    public function actionTaocanEdit($id = null)
    {
        $taocan = Taocan::findOne(['id' => $id]);
        if (!$taocan) {
            $taocan = new Taocan();
        }
        $form = new TaocanForm();
        ...
        $requirement_list = Requirement::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->all();
        $level_list = PhotographerLevel::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->all();


        return $this->render('taocan-edit', [
            'list' => $taocan,
    ...
    {
        $taocan_list = Taocan::find()->alias('t')->leftJoin(Requirement::tableName().'r','r.id=t.requirement_id')
            ->leftJoin(PhotographerLevel::tableName().'p','p.id=t.level_id')
            ->where(['t.store_id' => $this->store->id, 't.is_delete' => 0])
            ->select(['t.*','r.name as r_name','p.name as level_name'])
            ->orderBy('t.addtime DESC')->asArray()->all();


        return $this->render('taocan', [
            'taocan_list' => $taocan_list,
        ]);
    }

    public function actionTaocanDel($id=0){
         $taocan=Taocan::findOne($id);
        if (!$taocan) {
            return [
                'code' => 1,
                'msg' => '套餐删除失败或已删除',
        ...
        foreach ($taocan->errors as $errors) {
            return [
                'code' => 1,
                'msg' => $errors[0],
            ];
     ...
     * 分类编辑
     */
    public function actionRequirementEdit($id = null)
    {
        ...
        return $this->render('requirement-edit', [
    ...
    {
        $cat_list = PhotographerLevel::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->orderBy('addtime DESC')->all();
        return $this->render('photographer-level', [
            'cat_list' => $cat_list,
        ]);
    }
     ...
     * 分类编辑
     */
    public function actionPhotographerEdit($id = null)
    {
        $photographerLevel = PhotographerLevel::findOne(['id' => $id]);
        if (!$photographerLevel) {
            $photographerLevel = new PhotographerLevel();
        }
        $form = new PhotographerLevelForm();
        if (\Yii::$app->request->isPost) {
            $model = \Yii::$app->request->post('model');
            ...
            $form->level = $photographerLevel;

            return $form->save();
        }


        return $this->render('photographer-edit', [

            'list' => $photographerLevel,
        ]);
    }
     ...
     * 分类删除
     */
    public function actionRequirementDel($id)
    {
        $dishes = Cat::findOne(['id' => $id, 'is_delete' => 0]);
        if (!$dishes) {
            return [
