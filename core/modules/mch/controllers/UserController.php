...
use app\modules\mch\models\BalanceCashForm;
...
class UserController extends Controller
{
    ...
    public function actionRechangeLog($user_id = 0)
    {
        $query = IntegralLog::find()
            ->andWhere(['store_id' => $this->store->id, 'user_id' => $user_id, 'type' => 0]);
        $count = $query->count();
...
//            ->leftJoin(['current' => Level::tableName()], 'current.level=ro.current_level and current.is_delete=0')
        ...
        $p = new Pagination(['totalCount' => $count, 'pageSize' => 20]);
        $list = $query->select(['ro.*', 'u.nickname', 'after_name' => $afterQuery, 'current_name' => $currentQuery])
    ...
    {
        $cash = BalanceCash::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        if (!$cash) {
            return [
                'code' => 1,
                'msg' => '提现记录不存在，请刷新重试'
        ...
        $cash_cache = \Yii::$app->cache->get('bcash_cache_' . $id);
        ...
        \Yii::$app->cache->set('bcash_cache_' . $id, $cash->order_no);
        ...
        \Yii::$app->cache->set('bcash_cache_' . $id, false);
        if ($cash->save()) {
            /* if ($cash->status == 3) {
     ...
     * 支付未做
     */
    public function actionConfirm($id = 0, $status = 0,$is_down=0)
    {
        $cash = BalanceCash::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        if (!$cash) {
            return [
        ...
        if($is_down){
            ...
            $money_record->store_id = $cash->store_id;
            $money_record->user_id = $cash->user_id;
            $money_record->status = 0;
            $money_record->price=strval($cash->price);
            $money_record->type = 5;
            $money_record->addtime = time();
            $money_record->detail = "提现到微信";
            $money_record->save();
            $wechat_tpl_meg_sender = new CashWechatTplSender($this->store->id, $cash->id, $this->wechat);
    ...
    public function actionRechargeMoneyLog($user_id = null)
    {
        if(!$user_id){
            $url = \Yii::$app->urlManager->createUrl(['mch/user/index']);
            \Yii::$app->response->redirect($url)->send();
        }
        $query = IntegralLog::find()
            ->andWhere(['store_id' => $this->store->id, 'user_id' => $user_id, 'type' => 1]);
        $count = $query->count();
        $p = new Pagination(['totalCount' => $count, 'pageSize' => 20]);
        $list = $query->orderBy('addtime DESC')
            ->limit($p->limit)
