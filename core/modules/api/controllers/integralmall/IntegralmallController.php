...
class IntegralmallController extends Controller
{
    ...
    public function actionGetQrcode()
    {
        ...
        $order = IntegralOrder::findOne(['order_no'=>$order_no,'store_id'=>$this->store->id]);
        $form = new QrcodeForm();
        $form->data = [
            'scene'=>"{$order_no}",
            'page'=>"pages/integral-mall/clerk/clerk",
            'width'=>100
        ];
    ...
    public function actionClerk()
    {
        ...
        if($integralOrder->is_send == 0){
            return new \app\hejiang\ApiResponse(1, '订单待发货');
        }
