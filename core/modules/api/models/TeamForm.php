...
class TeamForm extends Model
{
    ...
    public function getOrder()
    {
        ...
        $sql = $select . $sql . " LEFT JOIN {$user_table} AS u ON u.id=al.user_id {$where}";
        if ($this->status == 0) {
        ...
        $list = \Yii::$app->db->createCommand($sql . "ORDER BY al.addtime DESC LIMIT {$pagination->limit} OFFSET {$pagination->offset}")->queryAll();
        ...
        foreach ($list as $index => $value) {
            ...
            if ($value['is_pay'] == 0) {
                $new_list[$index]['status'] = "待付款";
            } else if ($value['is_pay'] == 1 && $value['is_price'] == 0) {
                $new_list[$index]['status'] = "已付款";
            } else if ($value['is_price'] == 1) {
                $new_list[$index]['status'] = "已完成";
            }
            $share_setting = $this->share_setting;
    ...
    private function getSql()
    {
        ...
        $sql = " FROM ( {$sql_s} UNION {$sql_ms} UNION {$sql_pt} UNION {$sql_yy} ) AS `al` ";
