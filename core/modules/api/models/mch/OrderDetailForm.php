...
class OrderDetailForm extends Model
{
    ...
    public function search()
    {
        ...
        $express_list = Order::find()->alias('o')->select('o.express,COUNT(1) AS ecount')->where([
            'AND',
            ['o.mch_id' => $this->mch_id],
            ['IS NOT', 'o.express', null],
            ['!=', 'o.express', ''],
        ])->groupBy('o.express')
            ->limit(5)
            ->orderBy('ecount DESC')
            ->union(Express::find()->alias('e')->select('name AS express,sort')->limit(10))
            ->asArray()->all();

        $default_express = '';
        if ($order->express) {
            $default_express = $order->express;
        } elseif (count($express_list)) {
            $default_express = $express_list[0]['express'];
        }
        return [
            'code' => 0,
            'data' => [
