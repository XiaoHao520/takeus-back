...
class DetailSubmitForm extends Model
{
    public $store_id;
    public $user_id;
    public $id;
    ...
    private function checkHongbaoFinish($hongbao)
    {
        ...
        if ($hongbao->distribute_type == 0) {
            //随机分配
            if ($hongbao->coupon_total_money / $hongbao->user_num >= 5) {
                $all_min = 1;
            } else {
                $all_min = 0.01;
            }
            $max_money = $hongbao->coupon_total_money - ($hongbao->user_num - 1) * $all_min;
            $rest_money = $hongbao->coupon_total_money;
            foreach ($hongbao_list as $i => $item) {
                if ($i == (count($hongbao_list) - 1)) {
                    $coupon_money = $max_money;
                } else {
                    $min = min($all_min, $max_money);
                    if ($this->getFloatLength($all_min) == 0) {
                        //不带小数的
                        $coupon_money = $this->myRand($min, $max_money);
                    } else {
                        //带2位小数
                        $coupon_money = $this->myRand($min, $max_money, true, 2);
                    }
                    $rest_money = $rest_money - $coupon_money;
                    $max_money = $rest_money - (count($hongbao_list) - $i - 2) * $all_min;
                }
    ...
    private function sendWechatMsg($args)
    {
        ...
        $user = User::find()->where(['id' => $args['user_id']])->select('')->one();
