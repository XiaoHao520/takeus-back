...
class PayNotifyController extends Controller
{
    ...
    private function wechatPayNotify($res)
    {
        ...
        switch ($orderNoHead) {
            ...
            case 'U':
                ...
                break;
            case 'S':
                return $this->SyOrderNotify($res);
                break;
            case 'R':
                return $this->RewardOrderNotify($res);
                break;
        }
     ...
     * 摄影订单回掉
     */
    private function SyOrderNotify($res)
    {
        ...
        if ($order->save()) {
            //支付完成之后，相关的操作
            $photographer_order = RequirementOrder::find()->alias('ro')
            ...
            $msg->from_id = $photographer_order['r_user'];
            $msg->detail = $photographer_order['nickname'] . '向您支付了' . $order->pay_price . '元订单费用，赶紧查看吧！';
            $msg->store_id = $order->store_id;
            $msg->type = 0;
            ...
            $form->order_type = 0;
            $form->notify();
            $user_coupon = UserCoupon::findOne(['id' => $order->coupon_id, 'is_use' => 0, 'is_expire' => 0, 'is_delete' => 0]);
            if ($user_coupon) {
     ...
     * 摄影订单回掉
     */
    private function RewardOrderNotify($res)
    {
        $order = Reward::findOne([
            'order_no' => $res['out_trade_no'],
        ]);
        if (!$order) {
            return;
        ...
        if ($order->save()) {
            //支付完成之后，相关的操作
            $photographer_order = Reward::find()->alias('rw')->leftJoin(RequirementOrder::tableName() . 'ro', 'ro.id=rw.order_id')
                ->leftJoin(Photographer::tableName() . 'p', 'p.id=ro.photographer_id')
                ->leftJoin(User::tableName() . 'u', 'u.id=ro.user_id')
                ->where(['rw.id' => $order->id])
                ->select('p.user_id as p_user_id,ro.user_id as r_user,u.nickname,ro.id')->asArray()->one();

            $msg = new MoneyMsg();
            $msg->user_id = $photographer_order['user_id'];
            $msg->from_id = $photographer_order['r_user'];
            $msg->detail = $photographer_order['nickname'] . '向您支付了' . $order->price . '元额外费用，赶紧查看吧！';
            $msg->store_id = $order->store_id;
            $msg->type = 1;
            ...
            $msg = new OrderMsg();
            $msg->store_id = $order->store_id;
            $msg->user_id = $photographer_order['p_user_id'];
            $msg->detail = '用户：' . $photographer_order['nickname'] . ' 向您支付了 ' . $order->price . '元服务费';
            $msg->addtime = time();
